...
function screenshotWindow() {
  ...
  if (process.env.DEV) {
    screenshotW.loadURL(process.env.APP_URL + '#screenshot')
  } else {
    // 否则加载本地的 index.html 文件
    screenshotW.loadFile('index.html')
  }

  // 如果是调试模式，打开开发者工具
  if (process.env.DEBUGGING) {
    screenshotW.webContents.openDevTools()
  } else {
    // 否则监听开发者工具打开事件，并在打开时关闭开发者工具
    screenshotW.webContents.on('devtools-opened', () => {
      screenshotW.webContents.closeDevTools()
    })
  }
}
...
app.on('ready', () => {
  ...
  globalShortcut.register('CommandOrControl+Shift+A', async () => {
    ...
    screenshotW.webContents.send('截图事件', screenSource, '截图成功')
  })
})
